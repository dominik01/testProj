...
namespace Webron\UploadBundle\Classes;
...
class WebronUpload {
    ...
    public function upload($file, $path){
        ...
        $classDownloader->makeDirIfNotExists($path);
        $file->move($path, $file->getClientOriginalName());
        return $path . $file->getClientOriginalName();
    }
    
    function process($data, $options = null, $initialize = true, $error_messages = null) {
    ...
    protected function trim_file_name($file_path, $name, $size, $type, $error,
        ...
        if (0 && function_exists('exif_imagetype')) {
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
            ...
            if ($file_size === $file->size) {
                $file->url = $this->get_download_url($file->name);
                if (0 && $this->is_valid_image_file($file_path)) {
    ...
    public function post($print_response = true) {
		...
		if(!empty($this->name)) $file_name = $this->name;
        ...
        $files = array();
        if ($upload && is_array($upload['tmp_name'])) {//toto sa pouziva
